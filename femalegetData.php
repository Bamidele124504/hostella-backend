...
$query = "SELECT femalestudents.id, femalestudents.studentName, femalerooms.name AS name
          FROM femalestudents
          JOIN femalerooms ON femalestudents.room_id = femalerooms.id
          WHERE femalestudents.studentName = '$studentName'";
...
if ($result) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $otherQuery = "SELECT femalestudents.studentName
                       FROM femalestudents
                       JOIN femalerooms ON femalestudents.room_id = femalerooms.id
                       WHERE femalerooms.name = '$roomName'
                       AND femalestudents.studentName != '$studentName'";
